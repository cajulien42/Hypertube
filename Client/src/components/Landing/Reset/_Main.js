...
const ResetSmurt = (props) => {
  ...
  useEffect(() => {
    axios.get(`http://localhost:3000/API/users/infos/${props.params._id}`, { headers: { "x-auth-token": props.params.token } })
      .then((res) => {
        // console.log(res);
  ...
  */
  const submit = (e) => {
    ...
    if (newPwd.formIsValid === true && Object.keys(newDatas).length === 8 && error === false) {
      ...
      axios.put(`http://localhost:3000/API/users/update_password/${props.params._id}`, newDatas, { headers: { "x-auth-token": props.params.token } })
        .then((res) => {
          // console.log(res);
          if (res.data.success) {
            // console.log('success');
