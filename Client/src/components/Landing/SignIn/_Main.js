...
const SignInSmurt = (props) => {
  ...
  */
  const sendResetMail = (e) => {
    e.preventDefault();
    // console.log('sendResetMail() triggered');
    setLoading(true);
    ...
    axios.get(`http://localhost:3000/API/users/sendReset/${username}`)
      .then((res) => {
        // console.log(res);
        ...
        .then((res) => {
          ...
          if (token !== null && res.data.success) {
            ...
            setInfos((prevState) => {
              ...
              newOrderForm[0].config.touched = false;
  ...
  */
  useEffect(() => {
    if (error === true && infos.orderForm[0].config.touched === true) {
      setError(false);
    }
    if (error === 'ERROR' && resetForm.orderForm[0].config.touched === true) {
      setError(false);
    }
