...
const MovieCard = (props) => {
  ...
  const [hash, setHash] = useState(props.movie.torrents[0].hash)
  const [streamStatus, setStreamStatus] = useState('sleep'); // sleep or load or play
  const [url, setUrl] = useState(props.movie.trailer);
    ...
    return ( // Desktop Version 
      <div>
        <div className="clickable_area" /* Handle display off */
          ...
          style={{ width: `${window.innerWidth + 1000}px`, height: `${window.innerHeight + 1000}px`, }}></div>
        {/*---- Card ----*/}
        <div className="Movie_card" style={{ marginLeft: Expand ? "-10%" : "10%", backgroundImage: `url(${props.movie.mediumCover})` }}>
          <div className="Movie_card_inside">
            ...
            <div className="SimpleRow" style={{ justifyContent: "space-between" }}>
              ...
              {/* Title */}
              <h3 style={{ width: "80%", overflow: "hidden", fontSize: size, fontWeight: "700", marginBottom: 0, marginLeft: "20px" }}>{props.movie.title}</h3>
              <img className="TopLogo" alt="" src={Expand ? CommentsFull : Comments} onClick={HandleExpand}></img>
            </div>

            {/* Director Name */}
            <h4 style={{ marginTop: "-10px", marginLeft: "40px", fontSize: "2em", marginBottom: "30px", textShadow: "0px 0px 10px rgba(255, 255, 255, .2)" }}>{addInfos.Director}</h4>

            {/*--- Middle Elements ---*/}
            <div className="SimpleRow">
              {/* Player */}
              <div style={{ width: "533px", height: "300px", backgroundColor: "black", marginLeft: "20px" }}>
                <CustomPlayer
                  url={url}
              ...
              {/* Right infos */}
              <div style={{ width: "39%", textAlign: "right", marginRight: "20px", marginTop: "-20px", }}>
                {/* Year of Production */}
                <h4 style={{ fontSize: "3em", }}>{props.movie.year}</h4>
                {/* Duration */}
                <h4 style={{ fontSize: "2em", fontWeight: "500", marginTop: "-15px", marginBottom: "20px" }}><strong style={{ fontSize: "1.3em", fontWeight: "bold" }}>{props.movie.runtime}</strong> min</h4>
                {/* Main Actors */}
                <p style={{ fontSize: "1.9em", color: "white", fontWeight: "700", marginLeft: "20%", width: "80%", lineHeight: "1.1" }}>{addInfos.Actors}</p>
              </div>
            </div>

            {/*--- Bottom Elements ---*/}
            <div className='SimpleRow streamingButtons'>
              <i onClick={() => setStreamStatus('load')} className='fas fa-play launch'></i>
              <TorrentButton hash={hash} setHash={setHash} torrents={props.movie.torrents} />
            </div>
            {/* Description */}
            <p style={{ width: "520px", marginTop: "30px", height: "200px", marginLeft: "20px", textAlign: "justify", color: "lightgrey", textShadow: "0px 1px 10px black" }}>{addInfos.Plot}</p>
            {/* Rating */}
            <h4 style={{ position: "fixed", right: "20px", bottom: "-10px", fontSize: "5em", marginTop: "-50px", color: "yellow" }}>{props.movie.rating}</h4>
          </div>
