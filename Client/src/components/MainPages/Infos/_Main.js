...
const UpdateUserInfos = (props) => {
  ...
  */
  useEffect(() => {
    ...
    axios.get(`http://localhost:3000/API/users/all_users/username`, { headers: { "x-auth-token": token.token } })
      .then((res) => {
        if (res.data.success) {
          let usernameList = [];
  ...
  */
    ...
    e.preventDefault();
    console.log('saveNewInfos() triggered');
    ...
    if (newInfos.formIsValid && edit === true) {
      ...
      const token = JSON.parse(localStorage.getItem('JWT'));
      axios.put(`http://localhost:3000/API/users/${refDatas._id}`, profilChanges, { headers: { "x-auth-token": token.token} })
        .then((res) => {
          console.log(res);
